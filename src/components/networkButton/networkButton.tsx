...
export const NetworkButton = (props: any) => {
	...
	const [options, setOptions] = useState([
		{ icon: bscLogo, label: 'BSC', disabled: false, chainID: ETHEREUM_CHAIN_ID },
	]);
		...
		<Col roundTop>
			<DropDownButton className={`HelloTest ${uniqueClickClass}`} onClick={handleDropDownButton}>
				<StyledNetworkBtn className={`HelloTest`}>
					<NetworkIcon
						className={`HelloTest ${uniqueClickClass} `}
						src={selectedNetwork?.icon}
						alt={selectedNetwork?.label}
						width={logoSize}
					/>
					<Spacing horizontal="s" />
					<Text className={`HelloTest ${uniqueClickClass}`} fontSize="s">
								...
								<OptionButton key={item.label} onClick={() => switchNetwork(item)}>
									<LogoWrapper align="center" size={logoSize}>
										<NetworkIcon
											src={item.icon}
											alt={item.label}
											width={item.label === 'NEAR' ? logoSize * 0.8 : logoSize}
										/>
									</LogoWrapper>
									<Spacing horizontal="s" />
